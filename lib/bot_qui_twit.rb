...
def twit_to_a_list(client)
  #je récupère ma liste de personne et je la met sous un tableau
  ...
  # je choisis 5 personne au hasard en utlisant la fonction sample
  # tableau.sample(nombre de valeur aléatoire je veux)
  list_of_5 = (journalist.sample(5))
  #je split mon tableau pour différencier les valeurs
  list_of_5.to_s.split(',').delete('"')
  #je crée une boucle pour quand dans ma liste de 5 ca envoie a chacun

i=0
while i<list_of_5.size

twitter_user = list_of_5[i]
i += 1
#puts "Bonjour #{twitter_user}, pensez-vous que le célèbre rappeur VALD à raison de vouloir inculquer à son jeune public le repects des formules de politesse notamment en disant 'Bonjour' lorsqu'on s'adresse à quelqu'un? #bonjour_monde @the_hacking_pro"

#j'inclue le client update dans ma boucle pour qu'il l'envoie a i personnes
# ligne suivante permet de tweeter sur ton compte

#client.update("Bonjour #{twitter_user}, pensez-vous que le célèbre rappeur VALD à raison de vouloir inculquer à son jeune public le respect des formules de politesse, notamment en disant 'Bonjour' lorsqu'on s'adresse à quelqu'un? #bonjour_monde @the_hacking_pro" )

end
end

def perform
  #je fait appelle a ma fonction prennant en compte ma methode identifiant
  twit_to_a_list(identifiant)
end
perform
