...
#je crée une def qui me connect
def login
...
#je crée une def qui lance un streaming afin de suivre en live les actualités
def login_stream
...
#la def pour test de tweet via ruby
def test_tweet (client)
	# ligne qui permet de tweeter sur ton compte
	client.update('Mon premier tweet en Ruby (french) !!!!')
end

# la method du bot qui twit a un ensemble de personne
def hello_journalists(client)
	5.times do
		random = rand(LIST_JOURNALISTS.size)
		client.update("Bonjour #{LIST_JOURNALISTS[random]}, pensez-vous que le célèbre rappeur VALD à raison de vouloir inculquer à son jeune public le repects des formules de politesse notamment en disant 'Bonjour' lorsqu'on s'adresse à quelqu'un? #bonjour_monde @the_hacking_pro")
		puts "the bot sent an tweet to #{LIST_JOURNALISTS[random]} #bonjour_monde" #se puts me permet de savoir dans le terminal a qui il a posté un tweet
	end
end

#la méthod qui permet de liker les 25 dernier tweet du  #bonjour_monde a 25 personnes
def like_bonjour (client)
	client.search('#bonjour_monde').take(25).each do |tweet|
		client.fav tweet #le .fav est le raccourcit pour .favorite
		puts "The bot liked that tweet : #{tweet.text}"
	end
end

#la method pour folow les gens qui twit dans bonjour_monde
...
#la method pour folow et like en direct les personnes qui twit dans le bonjour_monde
def stream (clientStream,client) #je dois me connecter a clent et a client stream
	clientStream.filter(track: "#bonjour_monde") do |object| #je filtre ce que je veux suivre et je mets ce que je suis aprés le "track:"
		if object.is_a?(Twitter::Tweet) then #si l'objet est un tweet
			print "Nouveau Tweet de : @#{object.user.screen_name} : " #alors dis moi dans le terminal de qui est le tweet et ce qu'il a tweeter
	  		puts object.text
	  		client.follow(object.user) # folow le mec qui a poster le tweet
	  		client.fav object # like le tweet qui est posté
  		end
  end
